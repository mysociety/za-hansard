    ...
    def forwards(self, orm):
        ...
        for pca in orm['za_hansard.PMGCommitteeAppearance'].objects.all():
            ...
            for pca_field_name, pcr_field_name in self.field_name_mapping:
                pca_field = getattr(pca, pca_field_name)
                pcr_field = getattr(pcr, pcr_field_name)
                if pca_field and pcr_field and (pca_field != pcr_field):
                    raise Exception, "Contradictory value found"
                if pca_field:
                    setattr(pcr, pcr_field_name, pca_field)
                    pcr.save()

    def backwards(self, orm):
        "Write your backwards methods here."

        for pcr in orm['za_hansard.PMGCommitteeReport'].objects.all():
            for pca in pcr.appearances.all():
                for pca_field_name, pcr_field_name in self.field_name_mapping:
                    pcr_field = getattr(pcr, pcr_field_name)
                    setattr(pca, pca_field_name, pcr_field)
                    pca.save()
